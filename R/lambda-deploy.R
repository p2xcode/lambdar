...
#'
#' @param ecr_repo A name for the AWS ECR repo
#' @param image_tag A name for the image tag in the AWS ECR repo and lambda function
...
build_lambda <- 
  ...
  tdir <- tempdir()
  folder <- file.path(tdir, paste(ecr_repo, image_tag, sep = "-"))
  ...
  tryCatch(
    expr = {
      create_lambda_image(folder = folder, ecr_repo = ecr_repo, image_tag = image_tag)
    },
    error = function(e) {
...
#'
#' @param ecr_repo The tag of an existing local image tagged with ECR repo (see `build_lambda`)
#' @param image_tag The tag of the image in the AWS ECR repo
...
test_lambda <- function(ecr_repo, image_tag = "latest", runtime_function = "lambda_handler", payload = NULL) {
  ...
  repo_tag <- paste(repo_uri, image_tag, sep = ":")
...
deploy_lambda <-
  function(ecr_repo, image_tag = "latest", runtime_function = "lambda_handler", set_aws_envvars = FALSE, ...) {
    ...
    ecr_image_uri <- tryCatch(
      expr = {
        push_lambda_image(ecr_repo = ecr_repo, image_tag = image_tag)
      },
      error = function(e) {
